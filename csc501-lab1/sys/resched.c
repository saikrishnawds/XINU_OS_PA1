...
int resched()
{
	...
	int next_proc;        
		...
		next_proc = pick_nextproc(expdev_random);
            /* When a process is scheduled, it is taken out of the readyqueue by dequeue, so it is no longer in the readyqueue until the next 
 		time it preempts and it is reinserted in the queue, but if the second time a random process is created which is less than the 
		current process but since it is not found in the queue, the process next to it will be retruned from the pick_nextproc(), 
		but we expect the same process to be picked based on the random number generated, so to account for that in this condition 
		the process that is currently running is allowed to run for another quantum.*/
		...
		if((optr->pstate != PRCURR || optr->residual_time == 0) && (proctab[next_proc].goodness == 0))
		{
			...
			if(proctab[next_proc].goodness == 0){
				...
				else
				{
					...
					next_proc = find_next_proc();		
					//check if NULLPROC needs to be handled here?
