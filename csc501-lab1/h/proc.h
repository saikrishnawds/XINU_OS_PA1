...
struct	pentry	{
	...
	int	pwaitret;
	int time_alloted;		//This is a variable that keeps track of the time each process can run
					//in a single epoch
	int goodness;			//Goodness is the metric to decide which process needs to be scheduled.
	int residual_time;		//This is to store the remaining time for a process when it is preempted
					//before its time quantum in the current epoch.
};
